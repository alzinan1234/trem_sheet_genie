...
const Step1PriorInvestment: React.FC<Step1Props> = ({ data, onContinue, onStepBack }) => {
  ...
  const pricedRoundOptions = useMemo(() => {
    const names = formData.pricedRounds.map((round: PricedRound, index: number) =>
      (round.roundName || `Round ${String.fromCharCode(65 + index)}`).trim()
    );
    return names.filter((name: string, index: number) => name && names.indexOf(name) === index);
  }, [formData.pricedRounds]);
  ...
  const validateAndContinue = () => {
    ...
    formData.pricedRounds.forEach((r: PricedRound) => {
      ...
      if (!Number(r.investmentAmount) || Number(r.investmentAmount) <= 0) errors[`${p}_amount`] = 'Must be greater than $0';
    ...
    formData.safeNotes.forEach((s: SafeNote) => {
      ...
      if (s.investmentDate && s.convertingPricedRound) {
        const target = formData.pricedRounds.find((r: PricedRound) => r.roundName === s.convertingPricedRound);
        if (target?.investmentDate && s.investmentDate >= target.investmentDate)
          errors[`${p}_date`] = `Must be before ${target.roundName} date (${target.investmentDate})`;
      }
    });
                      ...
                      <tr key={equity.id} className="hover:bg-[#f9fafb] border-b border-[#f3f4f6]">
                        ...
                        <td className="py-3 px-4">
                          <select
                            ...
                            className={`w-full h-9 rounded-md border px-3 text-[13px] focus:ring-1 focus:ring-[#3b82f6] focus:border-[#3b82f6] ${
                              normalizeDividendType(equity.dividendTiming) === 'Continuous'
              ...
              <div className="grid grid-cols-[0.85fr_1fr_0.75fr] gap-2 items-end mb-2">
                ...
                <div className="min-w-0">
                  ...
                  <select
                    ...
                    className={`h-8 w-full rounded px-2 text-[12px] outline-none ${
                      normalizeDividendType(round.dividends) === 'Continuous'
                ...
                <div className="min-w-0">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full h-8 rounded text-[12px] outline-none border ${
