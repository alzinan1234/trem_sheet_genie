...
export default function InvestorTopbar({ onBellClick, isCollapsed }: InvestorTopbarProps) {
  ...
  useEffect(() => {
    ...
    // ── 3. Listen for user:updated event fired by updateAvatar / updateMyProfile
    //       This makes the topbar react instantly without a page refresh
      ...
      <div className="relative">
        ...
        >
          ...
          <div className="relative w-10 h-10 rounded-full overflow-hidden bg-[#2D60FF] flex items-center justify-center shadow-sm">
            {user?.avatar ? (
              // key forces img remount when avatar URL changes so browser doesn't show stale cache
              <img
                key={user.avatar}
                src={user.avatar}
                alt="avatar"
          ...
          <>
            ...
            <div className="absolute right-0 top-14 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 z-20 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
              <div className="p-4 border-b border-gray-100">
                {/* Avatar in dropdown header also reacts to updates */}
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-8 h-8 rounded-full overflow-hidden bg-[#2D60FF] flex items-center justify-center flex-shrink-0">
                    {user?.avatar ? (
                      <img key={user.avatar} src={user.avatar} alt="avatar" className="w-full h-full object-cover" />
                    ) : (
              ...
              <div className="p-2">
                <button
                  onClick={() => { router.push("/investor-admin/settings"); setShowDropdown(false); }}
